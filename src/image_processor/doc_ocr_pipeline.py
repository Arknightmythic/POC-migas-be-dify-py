...
except ImportError:  # dijalankan langsung sebagai script (python doc_ocr_pipeline.py)
    from vision_provider import vision_chat, describe_provider, VisionTruncated
    from json_repair import loads_lenient
    from example_leak import find_example_leaks
...
def crop_logo_with_llm(image_b64: str, image_path: str, output_path: str = "temp_logo.png") -> str:
    ...
    try:
        raw  = ollama_chat(messages, max_tokens=200, json_mode=True)
        data = loads_lenient(raw)  # [B-45] toleran terhadap koma yang hilang
...
def phase1_analyse(image_b64: str) -> dict:
    ...
    cleaned = raw
    try:
        # [B-45] toleran terhadap koma yang hilang dari qwen2.5vl
...
""")
    ...
    return (
        ...
        f"CRITICAL:\n"
        # [B-47] Lihat catatan panjang di handwritten_pipeline.py: kalimat lama
        # "Follow ALL rules and EXAMPLE snippets exactly" membuat model
        # menyalin ISI contoh ke dalam hasil OCR.
...
def main():
    ...
    # Flag CLI dipertahankan sebagai override cepat: nilainya di-set ke env
    # sebelum panggilan pertama, supaya satu sumber kebenaran tetap env.
    ...
    parser.add_argument("--model",      default=None,
                        help="Override model vision untuk provider aktif")
